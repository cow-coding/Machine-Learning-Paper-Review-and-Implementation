...
def main(args):
    ...
    model = VASP(**params).to(args.device)

    optimizer = optim.Adam(model.parameters(), lr=args.lr)
    criterion = loss_function_vasp
        ...
        for idx, start_idx in enumerate(range(0, N, args.batch_size)):
            ...
            if idx % args.log_interval == 0 and idx > 0:
                print(f"| epoch {epoch:3} | {idx:4d}/{N//args.batch_size:4d} batches "
                      f"| loss {train_loss/args.log_interval:4.2f} | Now KLD: {kld:4.5f}")
                train_loss = 0.0
        print(f"| epoch {epoch:3} | loss {total_train_loss/(N//args.batch_size):4.2f} "
              f"| Avg KLD: {KLD / (N//args.batch_size):4.5f}")
        print("-" * 89)
