...
def main(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for idx, start_idx in enumerate(range(0, N, args.batch_size)):
            ...
            loss, bce, kld = criterion(recon_batch, data, mu, logvar)
            BCE += bce
            KLD += kld
...
def train_ease(args):
    ...
    dataloder = dataset.get_matrix()

    n_items = raw_data['item'].nunique()
    n_user = raw_data['user'].nunique()

    model = EASE(n_items, args.device).to(args.device)
    ...
    for epoch in range(1, epochs+1):
        ...
        for idx, start_idx in enumerate(range(0, n_user, batch_size)):
            end_idx = start_idx + batch_size
            data = dataloder[start_idx:end_idx].to(args.device)
            ...
            if idx % args.log_interval == 0 and idx > 0:
                print(f"| epoch {epoch:3} | {idx:4d}/{n_user//args.batch_size:4d} batches | "
                      f"loss {train_loss:4.5f}")

        print(f"| epoch {epoch:3} | loss {train_loss:4.5f}")
        print("-" * 98)
