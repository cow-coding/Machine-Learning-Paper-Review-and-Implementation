...
def inference_ease(args):
    ...
    dataloader = dataset.get_matrix().to(args.device)
    ...
    for user in tqdm(users):
        seen_list = np.array(seen_dic[user])
        user_enc = dataset.user_enc.transform([user])[0]
        data = dataloader[user_enc]

        output = model(data)
        output = output.cpu().detach().numpy()
        ls.append(output)
        pred = np.argsort(-output)
        pred = dataset.item_enc.inverse_transform(pred)
    ...
    np.save(os.path.join(args.np_output_dir, args.model+"_"+'ease.npy'), np.array(ls))
